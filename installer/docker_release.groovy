...
def replaceStrInDockerfile(absoluteFilePath, arch, os, version) {
  ...
  def newContent = ""
  def flags = ["x86": false, "x64": false, "aarch64": false]
  for (line in content.split("\n")) {
    ...
    } else if (line.contains("BINARY_URL=")) {
      for (e in flags) {
        if (e.value) {
...
node('ossworker' && 'dockerfile') {
  ...
  if (releaseCard.size() > 0) {
    ...
    if (params.RELEASE == "8") {
      jdkVersion = (tagName.split("_")[-1].split("-")[0] + "-" + fullVersionOutput.split("-")[-1].split("\\)")[0]).trim()
    } else if (params.RELEASE == "11") {
      jdkVersion = "jdk-" + jdkVersion
    } else if (params.RELEASE == "17") {
      jdkVersion = tagName.split("_jdk")[0].split("-")[-1].trim()
    }
    ...
    dir(workdir) {
      ...
      if (foundDockerfiles) {
        ...
        for (dockerfile in dockerfiles) {
          ...
          if (fileBaseName.contains(params.TYPE) && releaseVersion == params.RELEASE) {
