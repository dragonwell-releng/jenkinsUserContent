        ...
        stage('iterateHistory') {
            ...
            steps {
                script {
                    ...
                    sh "git clean -ffdx"
                    sh "./tfb --network-mode='host'  --database-host='172.31.141.248' --client-host='172.31.141.248' --server-host='172.31.141.247' --concurrency-levels 4 8 16 32 64 128 256 512  --pipeline-concurrency-levels 4 8 16 32 256 1024  --duration 30 --test ${test}"
                    def results = findFiles(glob: 'results/**/results.json')
                    sh "python3 prettytable_result_parser.py --files ${results[0].path} --data latencyAvg totalRequests"
